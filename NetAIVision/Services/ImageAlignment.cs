    ...
    internal class ImageAlignment
    {
        ...
        public static void InitTemplate(Bitmap templateBitmap)
        {
            if (templateBitmap == null)
                throw new ArgumentNullException(nameof(templateBitmap));
        ...
        public static Bitmap AlignToTemplate(Bitmap srcBitmap)
        {
            ...
            // 每隔10帧 或 没有历史矩阵 才重新计算一次
            frameCount++;
            bool needRecalc = (frameCount % 10 == 0) || (lastHomography == null);
            ...
            if (needRecalc)
            {
                ...
                var goodMatches = matches.Take(Math.Min(80, matches.Length)).ToArray();
                ...
                Mat mask = new Mat();
                var newH = Cv2.FindHomography(InputArray.Create(ptsSrc), InputArray.Create(ptsTemplate), HomographyMethods.Ransac);

                double error = ComputeAlignmentError(ptsTemplate, ptsSrc, newH);

                // 如果误差比上次好，或者还没矩阵，则更新
                if (lastHomography == null || error < lastAlignError * 1.2)
                {
                    lastHomography = newH;
        ...
        {
            if (H == null || H.Empty() || templatePts.Length == 0)
            ...
            {
                total += Math.Sqrt(Math.Pow(projected[i].X - templatePts[i].X, 2) +
                                   Math.Pow(projected[i].Y - templatePts[i].Y, 2));
            }
        ...
        public static Bitmap MatToBitmap(Mat mat)
        {
            ...
            else if (mat.Channels() == 3)
            {
                ...
                int length = rgbMat.Rows * (int)rgbMat.Step();
                byte[] buffer = new byte[length];
        ...
        #endregion Bitmap <-> Mat (无需 Extensions)
    }
}